...
- Codex가 남기는 PR 리뷰/후속 코멘트는 `[codex-review]`, `[codex-followup]`, `[codex-contract-sync]` 같은 visible prefix와 hidden marker를 함께 포함합니다.

`.github/pull_request_template.md`를 기본 템플릿으로 사용합니다.

## 7. PR 자동화 규칙

- PR 자동화는 `reviewDecision`만으로 다음 액터를 정하지 않습니다.
- 공용 classifier가 각 open PR의 `next_actor`를 계산합니다.
  - `codex_review`: current head에 Codex review marker가 아직 없음
  - `codex_followup`: current head에서 최신 Codex action 이후 human input 또는 unresolved human thread가 있음
  - `waiting_human`: current head에서 Codex가 이미 action 했고 newer human input이 없음
  - `done_for_now`: open PR queue 기준 추가 Codex action이 없음
  - `blocked`: linked issue가 blocked 상태이거나 명시적 blocker가 있음
- 새 ready issue 작업은 open PR이 있다는 사실만으로 막지 않습니다.
  - `codex_review` 또는 `codex_followup` PR이 있을 때만 PR work가 우선입니다.
  - `waiting_human` PR은 새 ready issue 작업을 막지 않습니다.

## 8. Contract Precedence

Codex는 아래 우선순위로 scope를 해석합니다.

1. human PR review comments / review bodies / review threads
2. PR 생성 이후의 human issue comments
3. issue body
4. PR body
5. Codex-authored review/comment text

human feedback가 current issue/PR contract를 명확히 바꾸는 경우, Codex는 issue body와 PR body를 직접 sync할 수 있습니다.

- 허용되는 자동 수정:
  - `Closes #...` -> `Refs #...`
  - stale AC / scope / verification 문구 정리
  - active UI/code path와 맞지 않는 계약 문구 정리
- 자동 sync 이후에는 source review/comment URL을 포함한 `[codex-contract-sync]` 코멘트를 남깁니다.
- human feedback가 애매하면 contract를 임의로 바꾸지 않고 `status:blocked` 또는 clarification comment로 되돌립니다.
