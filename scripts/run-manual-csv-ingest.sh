...
if [[ "$REMAINING_COUNT" == "0" ]]; then
  python3 - "$RUN_SUMMARY" "$CSV_PATH" "$WORKSPACE" "$PLANNED_IDS" "$SUCCEEDED_IDS" "$FAILED_ATTEMPTS" <<'PY'
import json
import sys
from pathlib import Path
...
while IFS= read -r VIDEO_ID; do
  ...
  if ! (cd "$ROOT_DIR/cli" && "${FETCH_CMD[@]}") >"$FETCH_LOG" 2>&1; then
    ...
    if grep -Eiq 'YouTube blocked|429|Too Many Requests' "$FETCH_LOG"; then
...
python3 - "$RUN_SUMMARY" "$CSV_PATH" "$WORKSPACE" "$PLANNED_IDS" "$SUCCEEDED_IDS" "$REMAINING_IDS" "$FAILED_ATTEMPTS" "$RUN_STARTED_AT" "$RUN_FINISHED_AT" "$RUN_PROCESSED" "$RUN_SUCCEEDED" "$RUN_FAILED" "$STOP_REASON" "$PROXY_MODE" "$PROXY_URL" <<'PY'
import json
import sys
from pathlib import Path
...
if [[ "$STOP_REASON" == "rate_limited" ]]; then
  exit 2
fi
